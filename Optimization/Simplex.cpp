...
pair<vector<double>, double> getResult(vector<double> c, vector<vector<double>> A, vector<double> b) {
    int m = A.size(), n = A[0].size();
...
int main() {
    vector<double> c = {5, 4, 5, 2, 1, 3};
    vector<vector<double>> A = {{2, 4, 3, 1, 0, 0},
                  {4, 2, 3, 0, 1, 0},
                  {3, 0, 1, 0, 0, 1}};

    vector<double> b = {52, 60,36};

    try {
        auto [sol, rs] = getResult(c, A, b);
