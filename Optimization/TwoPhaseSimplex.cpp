...
pair<vector<double>, double> twoPhase(vector<double> c, vector<vector<double>> A, vector<double> b) {
    int m = A.size(), n = A[0].size();
    ...
    for (int i = 0; i < m; ++i) {
        A2.push_back(vector<double>(table[i].begin() + 3, table[i].begin() + n + 1));
    }


    vector<vector<double>> table2 = simplex(c, A2, b, B, cb);

    printTable(table2);
    // Tìm nghiệm tối ưu
    vector<double> sol(n, 0);
    for (int i = 0; i < m; ++i) {
        sol[table2[i][0]] = table2[i][2];
    }

    double rs = 0;
    for (int i = 0; i < m; ++i) {
        rs += table2[i][1] * table2[i][2];
    }
...
int main() {
    vector<double> c = {-4, 3, 1, 0, 0};
    vector<vector<double>> A = {{4, 3, 4, 1, 0},
                                 {4, 1, 6, 0, 1}};
    vector<double> b = {4, 5};

    try {
        auto [sol, rs] = twoPhase(c, A, b);
